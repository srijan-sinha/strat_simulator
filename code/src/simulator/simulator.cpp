...
Simulator::Simulator(const boost::property_tree::ptree& pt)
		...
		parser_(closing_prices_csv_path_.c_str()),
...
void Simulator::run_sim() {
	...
	while(parser_.read_next_line()) {
		...
		if (timestamps_.at(ts_num - 1) < start_time_) continue;
		if (timestamps_.at(ts_num - 1) > end_time_) break;

		parser_.get_data(data_points_);
		for (auto [strat_name, strat]: strats_) {
			dump_holdings_to_file(strat_name, ts_num, curr_position_[strat_name]);
